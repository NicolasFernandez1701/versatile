    ...
    <div className="class-card" onClick={() => onClick(cls)}>
      <div className="class-header">
        ...
        <div className="class-actions">
          <button className="action-btn edit" onClick={(e) => { e.stopPropagation(); onEdit(cls.id); }}>
            <Edit2 size={18} />
          </button>
          <button className="action-btn delete" onClick={(e) => { e.stopPropagation(); onDelete(cls.id); }}>
            <Trash2 size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="class-details">
        <div className="detail-item">
          <Clock size={16} />
          <span>{getDayName(cls.day_of_week)} {cls.start_time.slice(0, 5)}</span>
        </div>
        <div className="detail-item">
          <Users size={16} />
          <span>{enrolledCount} / {cls.capacity} cupos</span>
        </div>
      </div>
      
      <div className="commission-tag">
        Comisión: {cls.teacher_commission_pct}%
      </div>
    </div>
  );
}
