...
interface PlanFormProps {
  ...
  onSubmit: (data: {
    ...
    is_active: boolean;
  }, activities: { activity_name: string; classes_per_week: number }[]) => Promise<void>;
  onCancel: () => void;
  loading?: boolean;
}

export function PlanForm({ initialData, availableClasses, onSubmit, onCancel, loading }: PlanFormProps) {
  const { showError } = useAlert();
  const [name, setName] = useState(initialData?.name || '');
  const [price, setPrice] = useState('');
  const [isActive, setIsActive] = useState(true);
  const [activities, setActivities] = useState<{ activity_name: string; classes_per_week: number }[]>([]);
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.plan_activities) {
        setActivities(initialData.plan_activities.map(a => ({
  ...
  const totalClassesPerWeek = activities.reduce((sum, act) => sum + Number(act.classes_per_week || 0), 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (invalidActivities.length > 0) {
      showError(`La actividad "${invalidActivities[0].activity_name}" no existe. Solo podés seleccionar actividades pre-cargadas de la lista.`);
      return;
    }
        ...
        <div className="activities-list">
          {activities.map((act, idx) => (
            <div key={idx} className="activity-row">
              ...
              <button type="button" className="icon-btn text-danger" onClick={() => removeActivity(idx)}>
                <Trash2 size={20} />
              </button>
            </div>
          ))}
        </div>
        <button type="button" className="btn-secondary" onClick={addActivity} style={{ marginTop: '10px' }}>
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <input 
              ...
              style={{ flex: 1 }}
            />
            <button type="button" className="btn-secondary" onClick={calculateSuggestedPrice} title="Calcular Sugerido" style={{ padding: '0 0.75rem' }}>
              <Calculator size={20} />
            </button>
          </div>
          <small className="text-secondary" style={{ display: 'block', marginTop: '0.25rem' }}>Precio sugerido: suma de precios base x 4</small>
        </div>
      </div>
