...
export function StudentClassesPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const consumed = resData.filter(r => r.date.startsWith(monthStr) && r.status !== 'cancelled').length;
  ...
  const handleBooking = async (classId: string, classDate: Date, action: 'enroll' | 'cancel', startTime: string, existingReservationId?: string) => {
    try {
  ...
  const classesByDay: Record<number, ClassEntity[]> = { 1: [], 2: [], 3: [], 4: [], 5: [], 6: [], 0: [] };
  ...
  return (
    <div className="page-container" style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '1.5rem' }}>
      <div className="page-header" style={{ marginBottom: 0 }}>
        ...
        <div style={{ background: 'var(--primary-color)', color: 'white', padding: '0.75rem 1.25rem', borderRadius: '12px', fontWeight: 600 }}>
          Créditos Mensuales: {planLimits.consumed} / {planLimits.limit}
        </div>
      </div>

      <div style={{ display: 'flex', gap: '2rem', flex: 1, alignItems: 'flex-start', overflowY: 'auto' }}>
        <div style={{ flex: 1, background: 'var(--surface-color)', borderRadius: '16px', border: '1px solid var(--border-color)', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          {[1, 2, 3, 4, 5, 6, 0].map(dayOfWeek => {
            ...
            return (
              <div key={dayOfWeek} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <h2 style={{ fontSize: '1.2rem', color: 'var(--primary-color)', borderBottom: '2px solid var(--border-color)', paddingBottom: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {dayClasses.map((cls) => {
                    ...
                    const reservation = reservations.find(r => r.enrollments?.class_id === cls.id && r.date === dateStr && r.status !== 'cancelled');
                    ...
                    return (
                      <div key={cls.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem', background: 'var(--background-color)', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                        <div>
                          ...
                          <p className="text-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', marginTop: '0.25rem' }}>
                            <Clock size={16} /> {cls.start_time.substring(0, 5)} hs - {cls.end_time.substring(0, 5)} hs
                          </p>
                        </div>
                        ...
                        <div>
                          {isPast ? (
                            <span style={{ color: 'var(--text-secondary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                              {reservation?.status === 'attended' ? <><CheckCircle size={18} color="var(--success-color)"/> Asististe</> : 
                               reservation?.status === 'absent' ? <><XCircle size={18} color="var(--error-color)"/> Faltaste</> : 
                               'Finalizada'}
                            </span>
                          ) : reservation ? (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                              ...
                              <Button 
                                variant="danger" 
                                onClick={() => handleBooking(cls.id, classDate, 'cancel', cls.start_time, reservation.id)}
                              >
                                Cancelar
                              </Button>
                            </div>
                            ...
                            <Button 
                              variant="primary" 
                              onClick={() => handleBooking(cls.id, classDate, 'enroll', cls.start_time)}
