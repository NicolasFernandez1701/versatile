...
export function StudentPlansPage() {
  ...
  return (
    <div className="page-container">
      <div className="page-header" style={{ marginBottom: '2rem', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Nuestros Planes</h1>
        <p className="text-secondary" style={{ maxWidth: '600px' }}>Elegí el plan que mejor se adapte a tu entrenamiento y descubrí tu mejor versión con nosotros.</p>
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', justifyContent: 'center', alignItems: 'stretch' }}>
        {plans.map((plan, index) => {
          ...
          const isHighlighted = isActive || (!activePlanId && (index === Math.floor(plans.length / 2) || plan.name.toLowerCase().includes('premium')));

          return (
            <div key={plan.id} style={{
              background: isHighlighted ? 'linear-gradient(145deg, var(--surface-color), var(--surface-hover))' : 'var(--surface-color)',
              border: isActive ? '2px solid var(--success-color)' : (isHighlighted ? '2px solid var(--primary-color)' : '1px solid var(--border-color)'),
              borderRadius: '24px',
              padding: '2rem',
              width: '100%',
              maxWidth: '350px',
              ...
              {isHighlighted && (
                <div style={{
                  ...
                  {isActive ? <><CheckCircle size={14} /> TU PLAN ACTUAL</> : <><Star size={14} fill="currentColor" /> RECOMENDADO</>}
                </div>
              )}

              <div style={{ textAlign: 'center' }}>
                <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem', color: isHighlighted ? 'var(--primary-color)' : 'var(--text-primary)' }}>{plan.name}</h3>
                <div style={{ fontSize: '2.5rem', fontWeight: 'bold' }}>
                  ${plan.price.toLocaleString()}
                </div>
                <div className="text-secondary" style={{ fontSize: '0.9rem', marginTop: '0.25rem' }}>por mes</div>
              </div>

              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ background: 'var(--success-color)20', color: 'var(--success-color)', borderRadius: '50%', padding: '4px' }}>
                    <Check size={16} />
                  </div>
                ...
                {plan.plan_activities && plan.plan_activities.length > 0 && (
                  <div style={{ paddingLeft: '2.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
                    {plan.plan_activities.map(act => (
                      <div key={act.id}>• {act.classes_per_week}x {act.activity_name}</div>
                    ))}
                  </div>
                )}
              </div>
              ...
              <Button 
                ...
                style={{ width: '100%', marginTop: '1rem', padding: '1rem', fontSize: '1.1rem', borderRadius: '12px' }}
