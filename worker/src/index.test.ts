...
vi.mock('@pushforge/builder', () => ({
  buildPushPayload: vi.fn().mockReturnValue({
...
import { sendPushNotification } from '@pushforge/builder';
    ...
    if (table === 'enrollments') {
      return {
        select: vi.fn((columns: string) => {
          const isPreClassQuery = columns.includes('classes!inner(id');
...
describe('sendDailySummaries', () => {
  ...
  it('returns early when current hour is not 08:00 studio time', async () => {
    ...
    await sendDailySummaries(supabase as never, vapidKeys, 'America/Argentina/Buenos_Aires');
  ...
  it('inserts notifications and sends pushes for students and teachers at 08:00', async () => {
    ...
    const enrollments = [
      {
        ...
        class_id: 'class-1',
        classes: { activity_name: 'Yoga', start_time: '09:00:00', teacher_id: 'teacher-1' },
      },
      {
        student_id: 'student-1',
        class_id: 'class-2',
        classes: { activity_name: 'Pilates', start_time: '10:00:00', teacher_id: 'teacher-2' },
      },
      {
    ...
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        body: 'Hoy tenés 2 clase(s): Yoga, Pilates',
      })
    );
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
    ...
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        user_id: 'teacher-1',
        type: 'daily_summary',
        body: 'Hoy tenés 2 clase(s): Yoga, Yoga',
      })
    );
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        user_id: 'teacher-2',
        type: 'daily_summary',
        body: 'Hoy tenés 1 clase(s): Pilates',
      })
    );
  });
...
describe('sendPreClassReminders', () => {
  it('notifies student and teacher for classes in the 60-minute window', async () => {
    ...
    await sendPreClassReminders(supabase as never, vapidKeys);
    ...
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        reference_id: 'class-1',
        title: '¡Tu clase empieza pronto!',
        body: 'Yoga empieza a las 09:45',
      })
    );
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('does not notify teacher when teacher is the same as student', async () => {
    ...
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({ user_id: 'student-1' })
    );
...
describe('sendPlanExpirationAlerts', () => {
  ...
  it('inserts plan expiration notifications for profiles on milestone days', async () => {
    ...
    await sendPlanExpirationAlerts(supabase as never, vapidKeys);
    ...
    expect(supabase.insertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        body: 'Tu plan vence en 1 día(s). Renová para seguir disfrutando.',
      })
    );
  });
...
describe('sendPush', () => {
  ...
  it('deletes subscription when push endpoint returns 410', async () => {
    vi.mocked(sendPushNotification).mockResolvedValue({ status: 410 } as Response);
    ...
    await sendPush(supabase as never, 'student-1', { title: 'Hi', body: 'Hello' }, vapidKeys);

    expect(supabase.deleteEqMock).toHaveBeenCalledWith('id', 'sub-1');
  });

  it('deletes subscription when push endpoint returns 404', async () => {
    vi.mocked(sendPushNotification).mockResolvedValue({ status: 404 } as Response);

    const subscriptions = [
      {
        id: 'sub-2',
        user_id: 'student-1',
        endpoint: 'https://push.test/missing',
        p256dh_key: 'p256dh-2',
        auth_key: 'auth-2',
      },
    ];
